...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (pledges.funding_goal-SUM(pledges.amount))*-1 FROM projects INNER JOIN pledges ON projects.id=pledges.project_id GROUP BY users.name  "
end
