...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, SUM(amount) projects.funding_goal FROM projects pledgesINNER JOIN pledges WHERE pledges.project_id = projects.id GROUP BY projects.title HAVING projects.id = pledges.project_id ORDER BY SUM(amount), projects.funding_goal > -1 DESC LIMIT 2;"
end
